...
- **D14** `GroupRoomCoordinator` (§P3b) = irmão 1:N do `BotChatRegistry`, mesma ordem de resolução por membro (mapa local → scan → null → `adopt`). **O marcador é o TÍTULO, nunca `pinned`** — e isso sustenta peso: o slot `pinned` de um bot já significa "este é o meu chat canônico 1:1" (§D1), então pinar a thread de uma sala faria a conversa de grupo emergir como o chat privado daquele bot no próximo reinstall — um marcador com dois significados. O rename é diferido pela MESMA razão do pin (§V3): é o mesmo `PATCH` e falha igual. Scan pula `source=cron` (§V12).
- **D13** Sala de grupo (§P3a) = `GroupRoom{id, name, members, sessions}` persistida em `ServerStoreState.groupRooms` — o **1:N** que §D1 prometeu: o mapa 1:1 fica onde está e este nasce ao lado, sem migração. Thread de cada membro titulada `Group: <nome>` no gateway, que é o que torna a sala recuperável após reinstalação (papel que o `pinned` faz no canonical 1:1). Orquestração vive em `GroupRoundPolicy`, **pura**: menções escopam a rodada, quem fala nunca responde a si próprio, `@user` para a sala (responder seria falar por cima de quem foi chamado), e os caps (3 rodadas · 10 msgs/send) são duros.
...
| S9 | x | P4 — @mention autocomplete no composer (§D12) | PR #10 |
| S10 | x | P3a — fundação de sala: `GroupRoom` + persistência + `GroupRoundPolicy` (§D13). Sem UI, sem rede | PR #12 |
| S11 | x | P3b — `GroupRoomCoordinator`: thread por membro, título diferido, self-heal (§D14) | `feat/group-room-coordinator` |
...
| P3a | x | Fundação: modelo de sala, persistência 1:N, regras de rodada (§D13, §S10) | feito |
| P3b | x | `GroupRoomCoordinator`: adoção de thread por membro, título diferido (§D14, §S11) | feito |
| P3 | ~ | Group chats 2-6 bots: sala partilhada, ≤3 rodadas seriais, @menções escopam quem responde, `@user` escala pra ti (badge "needs you"), caps 10 msgs/send, sessão `Group: <nome>` por bot. Falta agora (c→d): orquestração ao vivo sobre WS (rodadas seriais reais), `GroupRoomScreen` + VM, criação/gestão de salas | 1-2 sem |
...
**Ordem:** P3 em curso — (a) e (b) feitos e **puros**; (c) é o primeiro que toca rede de verdade (rodadas seriais sobre WS) e o primeiro que precisa de smoke test no gateway vivo antes de merge.
